...
def load_weights(model,cfgfile,weightfile):
    ...
    # read first 5 header values
    np.fromfile(fp, dtype=np.int32, count=5)

    # The rest of the values are the weights
    blocks = parse_cfg(cfgfile)
    #loop over blocks and search for the specific layer
    for i, block in enumerate(blocks[1:]):
        #check if it's convolutional
        if (block["type"] == "convolutional"):
            ...
            #check if the convolutional is with or without batch normalization
            #with BN we need to take into account gamma, beta, mean, variance and conv weights
            #without BN we'll take into account conv biases and conv weights
